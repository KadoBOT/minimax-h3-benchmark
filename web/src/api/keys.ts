...
export const keys = {
  ...
  sharedGeneration: ["shared", "generation"] as const,

  runs: (params: RunListParams) => ["runs", params] as const,
  run: (id: string) => ["runs", "one", id] as const,
  sharedJobView: (id: string) => ["shared", "job-view", id] as const,
  ...
  arenaStandings: (minStars: number | null = 7) =>
    ["arena", "standings", minStars] as const,
